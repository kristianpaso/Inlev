...
(function(){
  ...
  function speakNumber(n, kind='milestone'){
    try{
      if(!st.voice) return;
      const u=new SpeechSynthesisUtterance(String(n));
      const v=pickSV(); if(v) u.voice=v;
      u.lang='sv-SE';
      if(kind==='milestone'){ u.rate=1; u.pitch=1; }
      else if(kind==='order'){ u.rate=0.95; u.pitch=0.85; }
  ...
  function onStats(stats){
    ...
    const prev = st.lastCount||0;
    const delta=c-prev;
    if(delta!==0){
      const prevH=st.hours[lbl]||0; st.hours[lbl]=Math.max(0,prevH+delta);
      st.total=c; st.error=ek; st.lastCount=c; save(st); renderLog();
      // Speak ONLY on milestones:
      if(delta>0 && MILESTONES.has(c)){ speakNumber(c,'milestone'); }
    }
    renderKPI(stats);
  }
  ...
  // Test-knapp: simulerar "Task 1 of N" där N=2..5 → läs upp N (1 talas inte)
  const testBtn = document.getElementById('testOrder');
