...
(function(){
  ...
  hdr.innerHTML = '<div class="brand">Inlev</div><nav class="nav">'
    ...
    + '<a href="/trav/" id="link-trav">Trav</a>'
    + '<a href="/bigplus/" id="link-bigplus">Bigplus</a></nav>';
  document.body.insertBefore(hdr, document.body.firstChild);
  var p = location.pathname.replace(/\/$/,'/');
  var t = document.getElementById('link-trav');
  var s = document.getElementById('link-sandningar');
  var b = document.getElementById('link-bigplus');
  if (p.startsWith('/bigplus')) b.classList.add('active'); else if (p.startsWith('/trav')) t.classList.add('active'); else s.classList.add('active');
})();
